...
describe("Cell", () => {
	it("should cascade overriden toString() methods nicely", () => {
		...
		expect(mirror.toString()).toEqual("Cell @ [Y:1, X:0] is unfrozen mirror (Phase: 0, Absorption: 0%) rotated 0°");
	});
	...
	it("should rotate element according to its element rotation angle increment", () => {
		...
		mirror.rotate(135);
		expect(mirror.toString()).toEqual("Cell @ [Y:1, X:0] is unfrozen mirror (Phase: 0, Absorption: 0%) rotated 135°");
		mirror.rotate(-270);
		expect(mirror.toString()).toEqual("Cell @ [Y:1, X:0] is unfrozen mirror (Phase: 0, Absorption: 0%) rotated 225°");
		detector.rotate(270);
